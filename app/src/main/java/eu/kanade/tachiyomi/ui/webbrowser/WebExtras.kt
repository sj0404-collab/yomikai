...
object WebStore {
    ...
    // v1.9.53: прозрачность по умолчанию 1.0 (полностью непрозрачное окно) —
    // так WebView с видео стабилен. Слайдер ниже меняет прозрачность только
    // фона рамки окна, не трогая содержимое (иначе видео останавливается).
    val miniWebOpen = mutableStateOf(false)
    val miniWebAlpha = mutableStateOf(1f)
